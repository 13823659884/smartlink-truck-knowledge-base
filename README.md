...
这是一个面向商用车用车、保养、维修、保用和故障诊断场景的本地知识库项目。系统解析企业文档，建立关键词索引、向量索引、任务分类和知识图谱，再由大模型根据检索证据生成带来源的回答。
...
- Kimi K3 或豆包回答模型负责阅读召回资料、推理和组织答案。
...
- 深度模式：适合复杂故障诊断，使用 Kimi K3 阅读更多证据并组织检查步骤。

回答模型可以替换，而不必重新生成知识库向量。向量是否需要重建取决于向量模型、维度和切片内容，而不是回答模型。
