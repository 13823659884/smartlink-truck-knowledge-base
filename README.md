...
桌面端对信息不完整的车辆专业问题采用“先确认、再回答”的两阶段交互：

1. 第一轮识别问题分类，并引导确认车辆系列、能源类型和本次回答重点。
2. 第二轮才执行知识库检索和模型生成，只回答用户选择的含义与标准、可能原因、排查处理、注意事项或完整诊断。

如果问题已经包含明确车型和查询方向，例如“JH6 的 SPN647 有哪些可能原因”，系统会跳过确认步骤直接回答。输入 VIN 且本地车辆主数据能够命中时，确认卡片会自动预填车辆信息。该流程不会减少知识库检索范围，只控制最终答案结构和长度；批量诊断继续使用自动分类流程，不进入交互确认。
...
| `scripts/document_importer.py` | 从桌面端导入单份资料并增量解析、分类和向量化 |
...
桌面端“资料导入”入口支持上传单份企业资料。后端会依次完成文件保存、内容解析、切片、任务分类、SQLite 索引更新和文字向量增量写入，并返回切片数量、向量化数量及失败信息。导入时选择的知识场景会写入文档与向量元数据，用于后续任务分类过滤；已有知识库和向量集合不会整体重建。

自动导入适合日常追加少量资料。首次构建、批量迁移或需要重建 PDF 页面及图片视觉向量时，仍建议使用前述离线构建脚本。上传资料、SQLite 数据库和生成的向量均保存在本地 `output/` 或配置的数据目录中，不会随 GitHub 源码提交。
...
当前桌面端启动的是豆包多模态向量版本：回答前先把用户问题转换为查询向量，并与 Qdrant 中的文字、PDF 页面和图片向量进行混合检索，再把召回正文和来源交给回答模型。桌面端还支持两阶段意图引导、车型与能源类型确认、定向回答、快速/深度模式、NDJSON 流式输出、连续追问、故障码精确检索、原文页码定位、知识图谱关系、反馈记录、资料增量导入和 Excel 批量诊断。
...
- `POST /api/import/document`：上传单份资料并自动解析、分类和增量向量化。
